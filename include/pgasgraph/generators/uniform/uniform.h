...
template <typename VertexData, typename EdgeData> struct Vertex
{
    ...
    {
        // std::cout << "(addNeighbour): id.partition=" << id.partition << ", id.id=" <<
        // id.partition
        //           << ", data=" << data << std::endl;
...
size_t Generate(const size_t vertexCount, double percentage, Graph<Vertex, Edge>& graph)
{
    ...
    {
        std::stringstream ss;
        ss << "[debug]: Starting generation of uniform random graph with " << vertexCount
           << " vertices and " << percentage << " density";
        //logMsg(ss.str());
        ss.flush();
    }
    ...
    while (!unconnected.empty())
    {
        ...
        if (u != v)
        {
            // logMsg("[DEBUG]: Adding edge between u=" + std::to_string(u.id) + " v=" +
            // std::to_string(v.id));
    ...
    {
        std::stringstream ss;
        ss << "[debug]: Finished generation of uniform random graphGeneration took "
