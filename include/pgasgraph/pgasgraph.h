...
Rank GetVertexParent(const typename Vertex::Id& id, const size_t verticesPerRank)
{
    ...
    {
        // logMsg("Parent for id.UniversalId()=" + std::to_string(id.UniversalId()) + " is parent="
        // + std::to_string(r));
    }
...
private:
    ...
    {
        id.id =
            id.UniversalId() - GetVertexParent<Vertex>(id, m_verticesPerRank) * VertexStoreSize();
        return id;
    }
};
    ...
    , m_verticesPerRank(verticesPerRank)
{
    ...
    upcxx::rpc(
        upcxx::rank_me(),
        [](GraphStorageType& graph, size_t verticesPerRank)
        { graph->resize(verticesPerRank, nullptr); },
        m_vertexStore,
        m_verticesPerRank)
        .wait();
...
template <typename Vertex, typename Edge> bool Graph<Vertex, Edge>::addEdgeHelper(Edge edge)
{
        ...
        return upcxx::rpc(
            ...
            [](GraphStorageType& vertexStore, Edge edge, Rank parentRank)
            {
                ...
                const std::size_t idx =
                    edge.from.UniversalId() -
                    GetVertexParent<Vertex>(edge.from, vertexStoreSize) * vertexStoreSize;
               assert(idx < vertexStoreSize);
                ...
                if (!vertexStore->operator[](idx))
                {
                    {
                        std::stringstream ss;
                        ss << "Adding vertex with id=" << edge.from.UniversalId()
                           << " at idx=" << idx << std::endl;
                    }
    ...
    auto fromFuture = insertSingleEdge(
        fromRank, m_vertexStore, edge, GetVertexParent<Vertex>(edge.from, m_verticesPerRank));
    auto toFuture =
        insertSingleEdge(toRank,
                         m_vertexStore,
                         edge.Invert(),
                         GetVertexParent<Vertex>(edge.Invert().from, m_verticesPerRank));
...
void Graph<Vertex, Edge>::ExportIntoFile(const std::string& fileName) const
{
    ...
    if (0 == upcxx::rank_me())
    {
        ...
        {
            std::string vertexStoreSerialized =
                upcxx::rpc(
                    r,
                    [](GraphStorageType& storage)
                    {
                        ...
                        for (auto idx{0}; idx < size; idx++)
                        {
                            ...
                            if (vertex)
                            {
                                ...
                                {
                                    graphStream << vertex->id.UniversalId() << " "
                                                << ngh.id.UniversalId() << std::endl;
                                }
                            }
                        }
