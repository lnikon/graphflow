...
namespace PGASGraph::Generators::Uniform
{
        ...
        Generate(const size_t vertexCount, double percentage, Graph<Vertex, Edge>& graph)
    {
        ...
        {
            std::stringstream ss;
            ss << "[debug]: Starting generation of uniform random graph with " << vertexCount << " vertices and " << percentage << " density";
            logMsg(ss.str());
            ss.flush();
        }
        ...
        {
            std::stringstream ss;
            ss << "[debug]: Finished generation of uniform random graphGeneration took " << std::chrono::duration_cast<std::chrono::milliseconds>(stopTime - startTime).count() << "ms" << std::endl;
            logMsg(ss.str());
        }
